...
const FilterModal = ({
  ...
  const handleMinInputChange = (e) => {
    ...
    setPriceRange((prev) => ({ ...prev, min: isNaN(minValue) ? 600 : minValue }));
  };

  const handleMaxInputChange = (e) => {
    const maxValue = parseInt(e.target.value, 10);
    setPriceRange((prev) => ({ ...prev, max: isNaN(maxValue) ? 30000 : maxValue }));
  };
  ...
  const handleFilterChange = () => {
    const filters = {
      ...
      propertyType,
      roomType: roomType === "Anytype" ? "" : roomType,
      amenities: [...amenities],
    };
    ...
    if (onApply) {
      ...
      onFilterChange("roomType", roomType === "Anytype" ? "" : roomType);
